...
class HelpPanelController(object):
    '''Pass-11: 按钮（home / up / back / next / search）已迁移到 headerbar，
    通过 self.headerbar.help_* 引用。本控制器不再依赖 view.back_button 等
    旧属性（已在 HelpPanelView 中删除）。'''
    ...
    def __init__(self, help_panel, view):
        ...
        self.headerbar.help_back_button.connect('clicked', self.on_back_button_clicked)
        self.headerbar.help_next_button.connect('clicked', self.on_next_button_clicked)
        self.headerbar.help_up_button.connect('clicked', self.on_up_button_clicked)
        self.headerbar.help_home_button.connect('clicked', self.on_home_button_clicked)
        self.headerbar.help_search_button.connect('toggled', self.on_search_button_toggled)
    ...
    def on_back_forward_list_changed(self, back_forward_list, item_added=None, items_removed=None):
        self.headerbar.help_back_button.set_sensitive(self.view.content.can_go_back())
        self.headerbar.help_next_button.set_sensitive(self.view.content.can_go_forward())
